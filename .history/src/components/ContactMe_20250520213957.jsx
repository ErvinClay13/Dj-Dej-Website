...
function ContactMe() {
  ...
  const sendEmail = (e) => {
    ...
    // ✅ Create or update hidden from_name input
    let fromNameInput = form.current.querySelector("input[name='from_name']");
    if (!fromNameInput) {
      fromNameInput = document.createElement("input");
      fromNameInput.type = "hidden";
      fromNameInput.name = "from_name";
      form.current.appendChild(fromNameInput);
    }
    fromNameInput.value = `${firstName} ${lastName}`;

    // ✅ Create or update hidden user_email input
    let userEmailInput = form.current.querySelector("input[name='user_email']");
    if (!userEmailInput) {
      userEmailInput = document.createElement("input");
      userEmailInput.type = "hidden";
      userEmailInput.name = "user_email";
      form.current.appendChild(userEmailInput);
    }
