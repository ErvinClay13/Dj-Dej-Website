...
function ContactMe() {
  ...
  const sendEmail = (e) => {
    ...
    let fromName = formEl.querySelector("input[name='from_name']");
    if (!fromName) {
    ...
    fromName.value = `${firstName} ${lastName}`;

    let userEmail = formEl.querySelector("input[name='user_email']");
    if (!userEmail) {
      userEmail = document.createElement("input");
      userEmail.name = "user_email";
      userEmail.type = "hidden";
      formEl.appendChild(userEmail);
    }
    userEmail.value = email;

    emailjs
      .sendForm("service_1ecvu9k", "template_bs8s2z2", formEl, {
