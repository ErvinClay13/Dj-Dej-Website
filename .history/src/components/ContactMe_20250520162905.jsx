...
function ContactMe() {
  ...
  const sendEmail = (e) => {
    ...
    // ✅ Create a hidden input for `from_name` (first + last)
    const firstName = form.current["first_name"].value;
    const lastName = form.current["last_name"].value;
    const fullName = `${firstName} ${lastName}`;

    const existingInput = form.current.querySelector("input[name='from_name']");
    if (existingInput) {
      existingInput.value = fullName;
    } else {
    ...
    emailjs
      ...
      .sendForm("service_1ecvu9k", "uoarmldLmdgLK6Idq", form.current, {
        publicKey: "uoarmldLmdgLK6Idq",
      })
      .then(() => {
